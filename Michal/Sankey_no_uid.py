...
def create_sankey(data):
    ...
    def get_link_color(row):
        return 'rgba(255,0,0,0.5)' if row['session_type'] == 'no_uid' else 'rgba(45,114,188,0.5)'  # Red for "no_uid", light blue otherwise
...
if __name__ == "__main__":
   ...
   select * from(
    ...
    (COUNT(*) / SUM(COUNT(*)) OVER (partition by sm_device_source,session_type)) * 100 AS transition_per,
    ...
    t2 as (select sid,sm_device_source,session_type from sessions where to_date(starttime)>=current_date()-3 
