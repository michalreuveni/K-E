...
def create_sankey(data):
    ...
    required_columns = {'from_page', 'to_page', 'transition_per', 'device', 'null_csid'}
    ...
    def get_link_color(row):
        return 'rgba(255,0,0,0.5)' if row[
                                          'null_csid'] == 'null_csid' else 'rgba(45,114,188,0.5)'  # Red for "no_uid", light blue otherwise
...
select * from(
    select from_page,to_page,count(*) as transition_count,sm_device_source as device,null_csid,
    (COUNT(*) / SUM(COUNT(*)) OVER (partition by sm_device_source)) * 100 AS transition_per,
    (COUNT(*) / SUM(COUNT(distinct sid)) OVER (partition by sm_device_source)) * 100 AS sid_per
    from (
    with t1 as (
    SELECT
        ...
        t2 as (select sid,csid,ifnull(sm_device_source,'none') as sm_device_source,
        case when csid is null then 'null_csid' else 'CSID_OK' end as null_csid
        from sessions where to_date(starttime)>=current_date()-7  and sid <> ''


   )

        select t1.*,t2.null_csid,t2.sm_device_source from t1 left join t2 on t2.sid=t1.sid where 
    ...
    data = query_on_multiple_cids(query, ['othello'])
