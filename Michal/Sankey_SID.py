...
def create_sankey(data):
    ...
    required_columns = {'from_page', 'to_page', 'transition_per', 'device', 'sid_change'}
    ...
    def get_link_color(row):
        return 'rgba(255,0,0,0.5)' if row['sid_change'] == 1 else 'rgba(45,114,188,0.5)'  # Red for "no_uid", light blue otherwise
    ...
    app.layout = html.Div([
        html.H1("Flow Diagram for Nebula including SID>CSID"),
        html.Div([
            dcc.Dropdown(
                id='device-dropdown',
                options=[
...
select * from(
    select from_page,to_page,count(*) as transition_count,sid_change,sm_device_source as device,
    (COUNT(*) / SUM(COUNT(*)) OVER (partition by sm_device_source)) * 100 AS transition_per,
    (COUNT(*) / SUM(COUNT(distinct sid)) OVER (partition by sid_change,sm_device_source)) * 100 AS sid_per
    from (
    with t1 as (
    SELECT
            ...
            LEAD(sid) OVER (PARTITION BY csid ORDER BY CONTEXT_START_TIME) AS next_sid,
            case when LEAD(application) OVER (PARTITION BY csid ORDER BY CONTEXT_START_TIME) is null then application else LEAD(application) OVER (PARTITION BY csid ORDER BY CONTEXT_START_TIME)
                end AS to_page,
            case when sid<>next_sid then 1 else 0 end as sid_change
        FROM contexts
        where to_date(starttime)>=current_date()-7),
        t2 as (select csid,ifnull(sm_device_source,'none') as sm_device_source from sessions where to_date(starttime)>=current_date()-7  and session_type='normal' and sid <> '' and lower(sm_site_source)='mexico'
        and sm_platform_type=2
        
   )
        
        select t1.*,t2.sm_device_source from t1 left join t2 on t2.csid=t1.csid where 
        t2.sm_device_source in ('ios','js','android')
    ) where
        
        
         from_page is not null 
        and to_page is not null 
        group by all) where transition_per>0.1
    ...
    data.to_csv('/Users/michal.reuveni/Desktop/Pycharm results/data_flows.csv')

    print(data['device'].unique())
#use nebula as default
    # Create the Sankey diagram
    sankey_app = create_sankey(data)
    sankey_app.run_server(port=8050, debug=True)
