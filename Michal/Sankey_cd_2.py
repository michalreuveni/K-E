...
def create_sankey(data):
    ...
    required_columns = {'from_page', 'to_page', 'transition_per', 'device', 'events'}
    ...
    def get_link_color(row):
        return 'rgba(255,0,0,0.5)' if row['events'] == "no_events" else 'rgba(45,114,188,0.5)'  # Red for "no_uid", light blue otherwise
    ...
    app.layout = html.Div([
        html.H1("Flow Diagram for bobi including e_elements_events=0"),
        html.Div([
            dcc.Dropdown(
                id='device-dropdown',
                options=[
...
if __name__ == "__main__":
    ...
    query = """
   select * from(
    select from_page,to_page,count(distinct sid) as sid_count,count(*) as transition_count,events,sm_device_source as device,
    (COUNT(*) / SUM(COUNT(*)) OVER (partition by sm_device_source )) * 100 AS transition_per,
    (COUNT(distinct sid) / SUM(COUNT(distinct sid)) OVER (partition by events,sm_device_source)) * 100 AS SID_per
    from (
    with t1 as (SELECT
    ...
    end as events from sessions where to_date(starttime)>=current_date()-3 
    and product='ATO' 
    
    and ifnull(lower(sm_site_source),'none')='anzretail'
    AND sid in (select associated_sid from api_calls
             where to_date(starttime) >=current_date()-3 ))
    select t1.*,t2.events,t2.sm_device_source from t1 inner join t2 on t2.sid=t1.sid
