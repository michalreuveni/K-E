...
def create_sankey(data):
    ...
    required_columns = {'from_page', 'to_page', 'transition_per', 'device', 'sm_application_version'}
    ...
    def get_link_color(row):
        return 'rgba(255,0,0,0.5)' if row['sm_application_version'] == '24.8.0' else 'rgba(45,114,188,0.5)'  # Red for "no_uid", light blue otherwise
...
SELECT DISTINCT context_array,URL_array, COUNT(*) AS total
FROM (
SELECT DISTINCT sm_platform_type,sid, ARRAY_AGG(application) within group (ORDER BY CONTEXT_START_TIME ASC) AS context_array, ARRAY_AGG(URL) within group (ORDER BY CONTEXT_START_TIME ASC) AS URL_array
FROM contexts
WHERE sid IN (
  SELECT sid 
  FROM sessions 
...
GROUP BY all order by total desc;
    ...
    data = query_on_multiple_cids(query, ['thor'])

    data.to_csv('/Users/michal.reuveni/Desktop/Pycharm results/journeys.csv')
    # Create the Sankey diagram
    sankey_app = create_sankey(data)
    sankey_app.run_server(port=8081, debug=True)
